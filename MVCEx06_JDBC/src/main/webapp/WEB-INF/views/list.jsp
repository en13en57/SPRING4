...
<head>
	...
	<script type="text/javascript">
		...
		function updateEmp(idx, name, role){
			// alert(idx + "\n" + name + "\n" + role);
			$("#cancelBtn").css('display','inline'); // 수정시 취소 버튼은 보이지 않는다.
		...
		// 취소버튼을 누르면
		function resetForm(){
			$("#cancelBtn").css('display','none'); // 수정시 취소 버튼은 보이지 않는다.
			$("#submitBtn").val("저장"); // 버튼의 제목이 "수정"으로 변경
			$("#idx").val(0);
			$("#mode").val("1");
			$("#name").val("");
			$("#role").val("");
		...
		<tr>
			<td colspan="4">
				<form action="insertOk" method="post">
					...
					<input type="hidden" name="mode" value="1" id="idx"/>
